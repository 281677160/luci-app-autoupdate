...
[[ ! -e /tmp/Version_Tags ]] && echo "因网络原因,未能检测到云端版本,请检查网络或您需要翻墙!" > /tmp/cloud_version && exit 1
...
if [[ ! -z "${CLOUD_Version}" ]];then
	...
	echo "没检测到云端固件，您可能把云端固件删除了，或您修改的Github地址错误，或格式不对称，比如爱快虚拟机安装固件不管什么安装格式都会变成Legacy引导!" > /tmp/cloud_version
fi
exit 0
