...
local github_url = luci.sys.exec("grep Github= /etc/openwrt_upgrade | cut -c8-100")
...
local current_version = luci.sys.exec("grep CURRENT_Version= /etc/openwrt_upgrade | cut -c17-100")
local current_model = luci.sys.exec("jsonfilter -e '@.model.id' < /etc/board.json | tr ',' '_'")
local firmware_type = luci.sys.exec("grep Firmware_Type= /etc/openwrt_upgrade | cut -c15-100")
